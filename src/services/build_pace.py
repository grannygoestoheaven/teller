...
from src.services.utils import _format_text_filename, _format_mp3_filename

storage = StorageBackend(use_bucket=env_settings.use_bucket, settings=env_settings)

def build_story(subject: str, pace: bool, narrative_style: str, difficulty: str) -> dict:
    ...
    # === Format filenames and folder names for later storing ===
    # story_foldername = subject
    # story_filename = subject
    story_foldername = _format_text_filename(subject, pace)
    story_filename = _format_text_filename(subject, pace)
    speech_filename = _format_mp3_filename(subject, pace)

    print(f"SPEECH : {speech_filename}")
    print (subject, narrative_style, difficulty)

    # === Generate story text and speech ===
    story_title, story_for_tts, story = generate_story_with_mistralai(subject, pace, narrative_style, difficulty) # returns text files
    # speech_filename, speech_audio = openai_tts(story_for_tts, subject) # one text file, one bytes file (mp3)
    speech_audio = openai_tts(story_for_tts) # one text file, one bytes file (mp3)
    # speech_filename, speech_audio = mistral_tts(story_for_tts, subject) # one text file, one bytes file (mp3)
    # speech_filename, speech_audio = elevenlabs_text_to_speech(story_for_tts, subject) # one text file, one bytes file (mp3)
    print(f"Generated speech filename: {speech_filename}")
    
    # === store files and get their paths ===
    json_story_filepath = storage.save_txt_to_json_file(story_foldername, story_filename, story_title, story_for_tts, story) # store the story parameters on the server and returns the clean story file
...
def load_story(subject: str, pace: bool, regenerate_mp3: bool) -> dict:
    ...
    try:
        ...
        story_for_tts = storage.get_tagged_text_from_json_file(story_filename)  # Add this helper if needed
        ...
        if regenerate_mp3 and not speech_url:
            ...
            speech_audio = openai_tts(story_for_tts)  # Your TTS function
            # speech_filename, speech_audio = mistralai_tts(story_for_tts, subject)  # Your TTS function
