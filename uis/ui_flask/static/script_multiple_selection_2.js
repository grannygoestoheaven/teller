    ...
    function handleMouseMove(event) {
        ...
        if (parent === chatHistory) {
            ...
            if (target.classList && target.classList.contains('word')) {
                ...
                // Determine highlighting based on vertical position
                // Using <= 2/3 and <= 1/3 for ranges simplifies the conditions
                if (mouseYRelativeToSpanTop <= (2/3) * spanHeight) { // Includes middle and top thirds
                    const nextWord = findNextWordSpan(target);
                    if (nextWord) {
                        nextWord.classList.add('highlight-word');
                        currentlyHighlightedWords.push(nextWord);

                        if (mouseYRelativeToSpanTop <= (1/3) * spanHeight) { // Includes just the top third
                            const thirdWord = findNextWordSpan(nextWord);
                            if (thirdWord) {
                                thirdWord.classList.add('highlight-word');
                                currentlyHighlightedWords.push(thirdWord);
                            }
                        }
                    }
                }
