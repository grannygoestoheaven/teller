...
function fadeVolume(el, from, to, duration, stepTime = FADE_STEP, onEnd) {
...
export function initAudioElements({ speech, background }) {
  ...
  // Ensure native volumes at defaults:
...
export function handleAudioPlayback(data) {
  ...
  // When speech ends: signal, then schedule background fade-out
  speechAudio.onended = () => {
    document.dispatchEvent(new CustomEvent('speechEnded', { detail: { storyText: data.story } }));
